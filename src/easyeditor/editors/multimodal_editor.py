...
class MultimodalEditor:
        ...
        **kwargs,
        ...
        if hasattr(self.hparams, "batch_size"):
            assert self.hparams.batch_size == 1, (
                "Single Edit, pls set the batch_size to 1...."
            )
        ...
        if sequential_edit:
            ...
            if (
                self.alg_name == "WISE"
                and hasattr(self.hparams, "save_path")
                and self.hparams.save_path
            ):
            ...
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                if self.alg_name == "IKE":
                    if self.hparams.k != 0:
                        ...
                        prefix_template = self.hparams.template.format(
                            prompt=request["prompt"], target=request["target"]
                        )
                        prompt_new_request["prompt"] = (
                            prefix_template + request["prompt"]
                        )
                        prompt_new_request["rephrase_prompt"] = (
                            prefix_template + request["rephrase_prompt"]
                        )
                        prompt_new_request["locality_prompt"] = (
                            prefix_template + request["locality_prompt"]
                        )
                        prompt_new_request["multimodal_locality_prompt"] = (
                            prefix_template + request["multimodal_locality_prompt"]
                        )
                        metrics = {
                            "case_nums": i,
                            "time": exec_time,
                            "post": compute_multimodal_hf_edit_results(
                                self.model,
                                self.model_name,
                                self.hparams,
                                self.tok,
                            ...
                            "pre": compute_multimodal_hf_edit_results(
                                self.model,
                                self.model_name,
                                self.hparams,
                                self.tok,
                    ...
                    if self.model_name in ["minigpt4", "blip2"]:
                        metrics = {
                            ...
                            "pre": compute_multimodal_edit_results(
                                self.model,
                                self.model_name,
                                self.hparams,
                                self.tok,
                ...
                if "locality_output" in metrics["post"].keys():
                    ...
                    base_logits = torch.tensor(metrics["pre"]["locality_output"]).to(
                        torch.float32
                    )
                    post_logits = torch.tensor(metrics["post"]["locality_output"]).to(
                        torch.float32
                    )
                    metrics["post"]["locality_acc"] = (
                        sum(post_logits.view(-1) == base_logits.view(-1))
                ...
                if "multimodal_locality_output" in metrics["post"].keys():
                    ...
                    base_image_logits = torch.tensor(
                        metrics["pre"]["multimodal_locality_output"]
                    ).to(torch.float32)
                    post_image_logits = torch.tensor(
                        metrics["post"]["multimodal_locality_output"]
                    ).to(torch.float32)
                    metrics["post"]["multimodal_locality_acc"] = (
                        sum(post_image_logits.view(-1) == base_image_logits.view(-1))
                        / post_image_logits.view(-1).shape[0]
                    )
        ...
        # single editing
            ...
            for i, request in tqdm(enumerate(requests), total=len(requests)):
                ...
                if self.alg_name == "IKE":
                    if self.hparams.k != 0:
                        assert "train_ds" in kwargs.keys(), (
    ...
    def edit_dataset(
        ...
        assert (
            sum(
                [
                    isinstance(ds, ds_in_dict)
                    for ds_in_dict in MULTIMODAL_DS_DICT.values()
                ]
            )
            > 0
        ...
        for i, request in enumerate(tqdm(ds, desc="Editing dataset", total=len(ds))):
            ...
            if "locality_output" in metrics["post"].keys():
                ...
                post_base_logits_softmax_top_k = torch.topk(
                    torch.nn.functional.softmax(post_logits, dim=-1), k=1, dim=-1
                ).indices
                metrics["post"]["locality_acc"] = (
                    sum(
            ...
            if "multimodal_locality_output" in metrics["post"].keys():
                ...
                post_image_logits = metrics["post"]["multimodal_locality_output"].to(
                    torch.float32
                )
                ...
                post_image_base_logits_softmax_top_k = torch.topk(
                    torch.nn.functional.softmax(post_image_logits, dim=-1), k=10, dim=-1
                ).indices
                metrics["post"]["multimodal_locality_acc"] = (
                    sum(
    ...
    def _prepare_requests(
        ...
        **kwargs,
        ...
        requests = [
            ...
            for prompt, target, image_, file_type_ in zip(
                prompts, targets, image, file_type
            )
        ]
        ...
        if rephrase_image is not None:
            ...
            rephrase_image = [
                self.vis_tok(file=m, file_type=n)
                for m, n in zip(rephrase_image, file_type)
            ]
        ...
        if "vision" in locality_inputs.keys():
            ...
            locality_image = [
                self.vis_tok(file=i, file_type="image") for i in locality_image_path
            ]
        ...
        if "loc_prompts" in kwargs:
            ...
            if len(kwargs["loc_prompts"]) < len(requests):
                kwargs["loc_prompts"] = (
                    kwargs["loc_prompts"]
                    * math.ceil(len(requests) / len(kwargs["loc_prompts"]))
                )[: len(requests)]
                random.shuffle(kwargs["loc_prompts"])
            assert len(kwargs["loc_prompts"]) == len(prompts)

            for i, request in enumerate(requests):
                request.update({"loc_prompt": kwargs["loc_prompts"][i]})
