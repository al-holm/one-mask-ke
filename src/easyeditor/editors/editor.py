...
class BaseEditor:
    ...
    def __init__(self, hparams: HyperParams):
        ...
        if type(self.model_name) is str:
            ...
            if "t5" in self.model_name.lower():
                ...
                self.model = AutoModelForCausalLM.from_pretrained(
                    ...
                    torch_dtype=torch_dtype
                    if hparams.alg_name not in ["MEND"]
                    else torch.bfloat16,
            ...
            if (
                ...
                and (
                    hparams.alg_name
                    not in ["ROME", "MEMIT", "EMMET", "R-ROME", "AlphaEdit"]
                )
            ):
                LOG.info(
            ...
            if (
                ...
                and (
                    hparams.alg_name
                    in ["ROME", "MEMIT", "EMMET", "R-ROME", "AlphaEdit"]
                )
            ):
                LOG.info(
        ...
        if (
            not hparams.model_parallel
            and hasattr(hparams, "device")
            and hparams.alg_name != "QLoRA"
        ):
        ...
        **kwargs,
        ...
        if hasattr(self.hparams, "batch_size") and not BatchEditor.is_batchable_method(
            ...
            assert self.hparams.batch_size == 1, (
                "Single Editing: batch_size should be set to 1"
            )
        ...
        **kwargs,
        ...
        for record_chunks in _chunks(requests, self.hparams.batch_size):
            ...
            if (
                ...
                self.alg_name == "LoRA"
                or self.alg_name == "QLoRA"
                or self.alg_name == "DPO"
            ):
                edited_model.unload()
            ...
            if self.alg_name == "IKE":
                ...
                if "locality" in all_metrics[idx]["post"].keys():
                    for locality_key in request["locality"].keys():
                        locality_result = []
                        if (
                            hasattr(self.hparams, "evaluation_type")
                            and self.hparams.evaluation_type == "LLM-judge"
                        ):
                            locality_result.append(
                                float(
                                    all_metrics[idx]["post"]["locality"][
                                        f"{locality_key}_output"
                                    ]
                                    == all_metrics[idx]["pre"]["locality"][
                                        f"{locality_key}_output"
                                    ]
                                )
                            )
                        ...
                        all_metrics[idx]["post"]["locality"][f"{locality_key}_acc"] = (
                            locality_result
                        )
                        all_metrics[idx]["post"]["locality"].pop(
                            f"{locality_key}_output"
                        )
        ...
        **kwargs,
        ...
        if "pre_edit" in kwargs and kwargs["pre_edit"] is not None:
            ...
            for i, request in enumerate(tqdm(requests)):
                ...
                if (
                    "locality" in request.keys()
                    and "loc_hop" in request["locality"].keys()
                ):
                    loc_results = []
        ...
        def post_edit_results(
            ...
            **kwargs,
            ...
            if self.alg_name == "IKE":
                ...
                all_results[idx].update(
                    {"case_id": idx, "requested_rewrite": request, "post": results_post}
                )
            if verbose:
                LOG.info(
                    f"{idx} editing: {request['prompt']} -> {request['target_new']}"
