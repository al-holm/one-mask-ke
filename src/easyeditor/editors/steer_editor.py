    ...
    def generate(
        ...
        **model_kwargs,
        ...
        assert (input_image is not None and self.modal_type == "mllm") or (
            input_image is None and self.modal_type == "llm"
        ), print("Error: llm cannot process image input or input_image is None.")
        ...
        if self.modal_type == "llm":
            ...
            inputs = self.tok(
                images=input_image, text=input_text, return_tensors="pt"
            ).to(self.device)
        ...
        # base decoding methods
            ...
            outputs = self.model.generate(
                **inputs, do_sample=True if temperature > 0 else False, **model_kwargs
            )

        output_text = self.tok.batch_decode(
