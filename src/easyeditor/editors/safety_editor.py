    ...
    def _locate_toxic_layer(self, model, tokenizer, requests, **kwargs):
        ...
        input = tokenizer(
            [
                ...
                for value in [pair["target_new"], pair["ground_truth"]]
            ],
            return_tensors="pt",
            padding=True,
            truncation=True,
        ...
        for j in range(len(requests)):
            ...
            for layer_index in range(1, len(hidden_states)):
                euclidean_distance = torch.dist(
                    hidden_states[layer_index][j * 2],
                    hidden_states[layer_index][j * 2 + 1],
                    p=2,
                )
        ...
        **kwargs,
        ...
        if "requests" in kwargs.keys():
            ...
            if prompts_with_systemPrompt is not None:
                requests_with_systemPrompt = self._prepare_requests(
                    prompts_with_systemPrompt,
                    target_new,
                    ground_truth,
        ...
        if "pre_edit" in kwargs and kwargs["pre_edit"] is not None:
            ...
            for i, request in enumerate(tqdm(requests)):
                metrics = {
                    "pre": compute_safety_edit_quality(
                        self.model,
                        self.tok,
                        request,
                        self.hparams.device,
                        max_tokens=self.hparams.max_length,
                        max_output_tokens=self.hparams.max_output_length,
                    )
                }
        ...
        if editing_method == "DINM":
            for i, (request, request_with_systemPrompt) in enumerate(
                zip(requests, requests_with_systemPrompt)
            ):
                start = time()
                if len(self.hparams.layers) == 0:
                    self.hparams.layers = self._locate_toxic_layer(
                        self.model,
                        self.tok,
                        [
                            request,
                ...
                all_metrics[i].update(
                    {
                        ...
                        "post": compute_safety_edit_quality(
                            edited_model,
                            self.tok,
                            request_with_systemPrompt,
                            self.hparams.device,
        ...
        **kwargs,
        ...
        if general_prompt is None:
            ...
            requests = [
                ...
                for prompt, ground_truth_, target_new_, general_prompt_ in zip(
                    prompts, ground_truth, target_new, general_prompt
                )
            ]
                ...
                for i, request in enumerate(requests):
                    if locality_inputs[locality_key]["prompt"][i] is not None:
                        request["locality"].update(
                            {
                                locality_key: {
                                    "prompt": locality_inputs[locality_key]["prompt"][
                                        i
                                    ],
                                    "ground_truth": locality_inputs[locality_key][
                                        "ground_truth"
                                    ][i],
                                }
                            }
