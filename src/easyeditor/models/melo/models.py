...
def get_tokenizer(config):
    ...
    tokenizer = getattr(transformers, config.model.tokenizer_class).from_pretrained(
...
def get_processor(config):
    processor_name = (
        ...
        if config.model.processor_name is not None
        else config.model.name
    )
    processor = getattr(transformers, config.model.processor_class).from_pretrained(
        processor_name, cache_dir=scr()
    )
    return processor
