...
class MeloConfig(PeftConfig):
    ...
    # melo_modified
    grace_layer: str = field(default=None, metadata={"help": "Module name as a grace layer"})

    grace_config: dict = field(default=None, metadata={"help": "Default settings of the grace layer"})
...
class VecDB:
    ...
    def update_cluster(self, index, new_key, new_value):
        ...
        self.table[index]["radius"] = max(
            largest_distance, torch.tensor(self.config["init_radius"], device=new_key.device).view(1)
        )
    ...
    def search_cluster(self, batch_query, smallest_distance_list, nearest_cluster_list):
        lora_mapping_block = []
        for query, smallest_distance, nearest_cluster in zip(
            batch_query, smallest_distance_list, nearest_cluster_list
        ):
            try:
...
class dynamic(nn.Module):
    def __init__(self, maximum_rank: int = 1, num_rank_per_block: int = 1):
        assert maximum_rank % num_rank_per_block == 0, "Maximum_rank % num_rank_per_block == 0 should be True"
...
class Linear(nn.Linear, LoraLayer):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            result += (self.lora_dropout[self.active_adapter](x) @ lora_A @ lora_B) * self.scaling[self.active_adapter]
