    ...
    **kwargs,
    ...
    with torch.no_grad():
        for w_name, (key_mat, val_mat) in deltas.items():
            key_mat, val_mat = (
                key_mat.to(f"cuda:{hparams.device}"),
                val_mat.to(f"cuda:{hparams.device}"),
            )
...
def execute_emmet(
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        layer_ks, targets, cov = (layer_ks.double(), targets.double(), cov.double())
        ...
        D = D + hparams.emmet_lambda * torch.eye(
            D.shape[0], dtype=D.dtype, device=D.device
        )  # to counter ill-conditioned D
        try:
