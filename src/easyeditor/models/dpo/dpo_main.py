    ...
    **kwargs: Any,
    ...
    for it in range(hparams.num_steps):
        ...
        for txt_batch, tgt_pos_batch, tgt_neg_batch in zip(
            ...
            tokens_pos = tok(
                full_prompt_pos, return_tensors="pt", padding=True, truncation=True
            )
            tokens_pos["labels"] = tokens_pos["input_ids"].clone()
            tokens_pos["labels"][tokens_pos["input_ids"] == tok.pad_token_id] = (
                mask_token
            )
            ...
            tokens_neg = tok(
                full_prompt_neg, return_tensors="pt", padding=True, truncation=True
            )
            tokens_neg["labels"] = tokens_neg["input_ids"].clone()
            tokens_neg["labels"][tokens_neg["input_ids"] == tok.pad_token_id] = (
                mask_token
            )
