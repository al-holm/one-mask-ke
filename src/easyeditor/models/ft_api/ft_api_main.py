...
def apply_ft_api_to_model(
    ...
    **kwargs,
    ...
    openai_file = openai.File.create(file=open(example_dir, "rb"), purpose="fine-tune")
    ...
    openai_job = openai.FineTuningJob.create(
        training_file=f"{openai_file['id']}", model=f"{hparams.model_name}"
    )

    start = time.time()
    while True:
        edited_model = openai.FineTuningJob.retrieve(f"{openai_job['id']}")[
            "fine_tuned_model"
        ]

        if edited_model is None:
            print(
                f"Waiting for openai to complete the fine-tuning task!!! Time Cost:{time.time() - start}s."
            )
            time.sleep(10)
        else:
            break
