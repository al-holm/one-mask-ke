...
def apply_deepedit_api_to_model(datasets, hparams: DeepEditApiHyperParams, **kwargs):
    # read hparams
    ## read api_key
    ...
    def get_sent_embeddings(sents, contriever, tok, BSZ=32):
        ...
        for i in range(0, len(sents), BSZ):
            ...
            inputs = tok(
                sent_batch, padding=True, truncation=True, return_tensors="pt"
            ).to("cuda")
            with torch.no_grad():
    ...
    def retrieve_facts(query, fact_embs, contriever, tok, k=2):
        inputs = tok([query], padding=True, truncation=True, return_tensors="pt").to(
            "cuda"
        )
        with torch.no_grad():
            outputs = contriever(**inputs)
    ...
    def oneStep(prompt, last_entity, new_entity, last_thought, count, fact_ls):
        ...
        for fact_sent_ in fact_sent_ls[::-1]:
            ...
            conflict_flag = (
                judgeConflict(fact_sent_, current_thought)
                and judgeConflict1(fact_sent_, current_thought)
                and judgeConflict2(fact_sent_, current_thought)
            )
    ...
    for d in tqdm(dataset[0:100]):
        ...
        for q in d["questions"][:1]:
            ...
            if ans.lower() == d["new_answer"].lower() or ans.lower() in [
                ans_.lower() for ans_ in d["new_answer_alias"]
            ]:
                cor += 1
