    ...
    **kwargs: Any,
    ...
    if hparams.use_icl_examples is True:
        ...
        query_embedding = util.normalize_embeddings(
            torch.tensor(sentence_model.encode(query_sentence, show_progress_bar=False))
            .unsqueeze(0)
            .to(device)
        )
...
def apply_ike_to_multimodal_model(
    ...
    **kwargs: Any,
    ...
    with open(
        f"{hparams.results_dir}/{hparams.alg_name}/embedding/"
        f"{hparams.task_name}_embeddings.pkl",
        "rb",
    ) as fIn:
        stored_data = pickle.load(fIn)
        stored_sentences = stored_data["sentences"]
...
def apply_ike_to_per_model(request, loc_request, tokenizer, device):
    ...
    outer_pre_inputs = [
        pre_icl_prompt_template.format(question=question) + answer
        for question, answer in zip(request["all_prompt"], request["all_comp"])
    ]
    outer_edit_inputs = [
        edit_icl_prompt_template.format(
            target_per=request["target_personality"],
            edit_topic=request["ent"],
            question=question,
        )
        + answer
        for question, answer in zip(request["all_prompt"], request["all_comp"])
    ...
    loc_pre_inputs = [
        pre_icl_prompt_template.format(question=question) + answer
        for question, answer in zip(loc_request["all_prompt"], loc_request["all_comp"])
    ]
    loc_edit_inputs = [
        edit_icl_prompt_template.format(
            target_per=request["target_personality"],
    ...
    inner_edit_q = edit_icl_prompt_template.format(
        target_per=request["target_personality"],
        edit_topic=request["ent"],
    ...
    same_per_mask = torch.tensor(
        [request["inner_per"][0] == o for o in request["all_per"]], device=device
    )
    example = {
