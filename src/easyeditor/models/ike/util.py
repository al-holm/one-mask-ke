...
def encode_ike_facts(
    sentence_model: SentenceTransformer, ds: Dataset, hparams: IKEHyperParams
):
    sentences = []
    ...
    with open(
        f"{base_path}/{safe_model_name}_{type(ds).__name__}_{len(ds)}.pkl", "wb"
    ) as fOut:
        pickle.dump(
...
def encode_ike_facts_multimodal(
    sentence_model: SentenceTransformer, ds: Dataset, hparams: IKEMultimodalHyperParams
):
    sentences = []
    for i, train_data in enumerate(ds):
