...
class KnowledgeNeurons:
    ...
    def _prepare_inputs(self, prompt, target=None, encoded_input=None):
        ...
        if target is not None:
            if (
                "qwen" in self.model_type
                or "gpt" in self.model_type
                or "t5" in self.model_type
                or "llama" in self.model_type
            ):
        ...
        for i in range(n_sampling_steps):
            ...
            if self.model_type == "t5":
                for q, target_idx_ in enumerate(target_idx):
                    ...
                    argmax_prob, argmax_id = [
                        i.item() for i in probs[:, q, :].max(dim=-1)
                    ]
    ...
    def scaled_input(activations: torch.Tensor, steps: int = 20, device: str = "cpu"):
        ...
        if activations.dim() == 2:
            ...
            return (
                tiled_activations
                * torch.linspace(start=0, end=1, steps=steps).to(
                    tiled_activations.device
                )[:, None]
            )
        elif activations.dim() == 3:
            tiled_activations = einops.repeat(
                activations, "b m d -> (r b) m d", r=steps
            )
            return (
                tiled_activations
                * torch.linspace(start=0, end=1, steps=steps).to(
    ...
    def get_coarse_neurons(
        ...
        if threshold is not None:
            ...
            if (
                self.model_type == "t5"
                and len(coarse_neurons) > 0
                and len(coarse_neurons[0]) == 3
            ):
                coarse_neurons = list(
                    set(
                        [
                            (layer_idx, neuron_idx)
                            for layer_idx, _, neuron_idx in coarse_neurons
                        ]
                    )
                )
    ...
    def get_scores_for_layer(
        ...
        if attribution_method == "integrated_grads":
            ...
            for i in range(n_sampling_steps):
                if i > 0 and (
                    "qwen" in self.model_type
                    or self.model_type == "gpt"
                    or self.model_type == "llama"
                ):
                    # retokenize new inputs
                    encoded_input, mask_idx, target_label = self._prepare_inputs(
                        prompt, ground_truth
                ...
                if self.model_type == "chatglm2":
                    ...
                    integrated_grads_this_step *= (
                        baseline_activations.squeeze(0) / steps
                    )
                else:
                    integrated_grads_this_step *= (
                        baseline_activations.squeeze(0) / steps
    ...
    def modify_weights(
        ...
        if mode == "edit":
            ...
            original_prediction_id = (
                argmax_tokens[0] if len(argmax_tokens) == 1 else argmax_tokens
            )
            ...
            if mode == "edit":
                if self.model_type == "gpt2" or self.model_type == "chatglm2":
                    if original_prediction_embedding.device != output_ff_weights.device:
                        original_prediction_embedding = (
                            original_prediction_embedding.to(output_ff_weights.device)
                        )
                    ...
                    if original_prediction_embedding.ndim > 1:
                        ...
                        output_ff_weights[position, :] -= (
                            original_prediction_embedding * 2
                        )
