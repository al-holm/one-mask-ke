...
class MalmenRewriteExecutor:
    ...
    def apply_to_model(
        ...
        **kwargs,
        ...
        assert len(requests) >= hparams.n_edits, (
            "The number of requests must be greater than or equal to the value of n_edits."
        )
        # Define i/o
        ...
        for i in range(hparams.n_edits // hparams.batch_size):
            ...
            sentences = [
                request["prompt"] + targets[i] for i, request in enumerate(batch)
            ]
