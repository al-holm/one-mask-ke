    ...
    def _deco_greedy_decoding(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            ].squeeze(dim=1)  # [10 layers, 10 candidate tokens]
    ...
    def _deco_sample_decoding(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            )  # (num_premature_layers, batch_size, num_features)
            candidate_tokens_early_exit_probs = softmax_early_exit_layers[
                :, :, candidate_tokens_ids
            ].squeeze(dim=1)  # [10 layers, 10 candidate tokens]
            max_candidate_tokens_idx = torch.argmax(candidate_tokens_early_exit_probs)
    ...
    def _deco_beam_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            multi_candidate_tokens_probs, multi_candidate_tokens_ids = torch.topk(
                last_layer_tokens_probs, dim=-1, k=threshold_top_k
            )
            multi_candidate_tokens_cumulative_probs = (
                multi_candidate_tokens_probs.cumsum(dim=-1)
            )
            tensor_list = []
            for seq_idx in range(multi_candidate_tokens_cumulative_probs.size(0)):
                candidate_tokens_indices = torch.searchsorted(
                    multi_candidate_tokens_cumulative_probs[seq_idx, :],
                    threshold_top_p,
                    right=False,
                )
                candidate_tokens_cutoff_idx = torch.min(
                ...
                candidate_tokens_ids = multi_candidate_tokens_ids[
                    seq_idx, :candidate_tokens_cutoff_idx
                ]
                stacked_early_exit_layers = torch.stack(
                    [dict_outputs[i][seq_idx, -1, :] for i in early_exit_layers], dim=0
                )
                softmax_early_exit_layers = F.softmax(
                    stacked_early_exit_layers, dim=-1
                )  # (num_premature_layers, batch_size, num_features)
                ...
                max_candidate_tokens_idx = torch.argmax(
                    candidate_tokens_early_exit_probs
                )
                premature_max_probs = candidate_tokens_early_exit_probs.max().item()
                target_layers = (
                    max_candidate_tokens_idx
                    // candidate_tokens_early_exit_probs.size(1)
                )

                selected_premature_layer_idx = early_exit_layers[target_layers.item()]
                selected_premature_layer_logits = dict_outputs[
                    selected_premature_layer_idx
                ][seq_idx, -1, :]  # [1, vocab_size]
                indices_to_remove = torch.ones_like(selected_premature_layer_logits)
                indices_to_remove[candidate_tokens_ids] = 0
                indices_to_remove = indices_to_remove.bool()
