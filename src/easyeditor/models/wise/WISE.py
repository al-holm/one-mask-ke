...
class WISE(torch.nn.Module):
    ...
    def edit(self, config, tokens, act_mask=None, deact_mask=None):
        ...
        if (
            ...
            print("Add New Weight to Memory...")
...
class WISEMultimodal(WISE):
    def edit(
        ...
        global merge_group_edit_history
        edit_history.append(
            [{f"{k1}": v1.to("cpu") for k1, v1 in text_tokens.items()}, False]
        )
        ...
        for i in range(config.n_iter):
            if i == 0:
                # --- we only need to create an optimizer for the first iteration (but forward pass instantiates the key, so optimzer is passed after first inference) ---
                optimizer = torch.optim.SGD(
                    [super().get_adapter_layer().new_weight],
                    config.edit_lr,
                    weight_decay=1e-5,
                )

            ft_loss = self._cal_ft_loss(
                multimodal_inputs, text_tokens, last_prompt_token_loc, ans_token_len
            )

            act_loss = super()._cal_activation_loss(
            ...
            if (
                self.config.retrieve
                and super().get_adapter_layer().merge_cnt > 0
                and self.config.replay
            ):
                memory_loss = []
                for _ in merge_group_edit_history:
    ...
    def _cal_ft_loss(
        self, multimodal_inputs, text_tokens, last_prompt_token_loc, ans_token_len
    ):
        if hasattr(self.model.config, "batch_size"):
            k = self.config.batch_size
        else:
