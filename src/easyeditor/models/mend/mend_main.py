...
class MendRewriteExecutor:
    ...
    def init_model(self, model, tok, params: MENDHyperParams):
        assert params.archive is not None or print("Training weights Needed....")

        def add_padding(tokenizer, model):
            tokenizer.add_special_tokens({"pad_token": "[PAD]"})
            model.resize_token_embeddings(len(tokenizer))
            model.transformer.wte.weight.data[-1] = (
    ...
    def apply_to_model(
        ...
        **kwargs,
        ...
        # Define i/o
        targets = [
            (" " if request["target_new"][0] != " " else "") + request["target_new"]
            for request in requests
        ]
        sentences = [
            request["prompt"] + targets[i] for i, request in enumerate(requests)
        ]
...
class MendMultimodalRewriteExecutor(MendRewriteExecutor):
    ...
    def apply_to_model(
        ...
        **kwargs,
        ...
        # Run MEND
        edit_inner = dict(
            image=image, text_input=text_input, labels=labels, prompts_len=prompts_len
        )
        # cond = {k: sent_tok[k] for k in ["input_ids", "attention_mask"]}
...
class MendPerRewriteExecutor(MendRewriteExecutor):
    ...
    def apply_to_model(
        ...
        **kwargs,
        ...
        self.alg.eval()
        edited_model, model_info = self.alg.edit(
            request["cond"], personality=True, return_factors=True
        )
