...
def compute_zs(
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        max_prob = (
            torch.exp((max_probs * mask).sum(1) / target_ids.size(0)).mean().item()
        )
...
def compute_z(
    ...
    rewriting_prompts, kl_prompts = (
        [
            ...
            + tok.decode(
                target_ids[:-1]
            )  # 这里好像是个bug，target_ids[:-1]是空的  tok.decode(target_ids[0])
            for context_types in context_templates
            for context in context_types
        ],
        ["{} is a"],
