...
def apply_pmet_to_model(
    ...
    **kwargs,
    ...
    deltas = execute_pmet(
        model, tok, requests, hparams, cache_template=cache_template
    )  # 存储了Equ14 左右的值
...
def execute_pmet(
        ...
        for rewrite_module_name in rewrite_module_names:
            ...
            if (
                ...
                try:
                    ...
                    z_list[rewrite_module_name].append(
                        torch.from_numpy(data["v_star"]).to("cuda")
                    )
                    data_loaded = True
            ...
            if not data_loaded:
                if len(rewrite_module_names) == 2:
                    ...
                    z_list[rewrite_module_names[0]].append(
                        cur_z_attn if "attn" in rewrite_module_names[0] else cur_z_mlp
                    )
                    z_list[rewrite_module_names[1]].append(
                        cur_z_attn if "attn" in rewrite_module_names[1] else cur_z_mlp
                    )
                    for rewrite_module_name in rewrite_module_names:
                        block_name = "attn" if "attn" in rewrite_module_name else "mlp"
                        cache_fname = (
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        for rewrite_module_name in rewrite_module_names:
            ...
            print(
                f"Writing {layers_ks[rewrite_module_name].size(0)} key/value pair(s) into layers"
            )
            ...
            upd_matrix = (
                (targets / np.sqrt((len(hparams.layers) - i)))
                @ layer_ks.T
                @ torch.inverse(
            ...
            with torch.no_grad():
                weights[weight_name][...] = weights_copy[
                    weight_name
                ] + upd_matrix.float().to("cuda:0")
...
def get_cov(
    ...
    try:
        return (
            torch.inverse(COV_CACHE[key].to("cuda:0"))
            if inv
            else COV_CACHE[key].to("cuda:0")
        )
    except:
        return (
            torch.inverse(COV_CACHE[key].to("cuda:1"))
            if inv
            else COV_CACHE[key].to("cuda:1")
        )
