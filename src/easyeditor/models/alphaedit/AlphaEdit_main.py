...
def apply_AlphaEdit_to_model(
    ...
    **kwargs,
    ...
    if not os.path.exists(hparams.P_loc):
        print("The null-space projection matrix P does not exist and now calculate.")
        ...
        if (
            ...
            P = torch.zeros(
                (len(hparams.layers), W_out.shape[0], W_out.shape[0]), device="cpu"
            )
        del W_out
...
def execute_AlphaEdit(
    ...
    for request in requests:
        ...
        if (
            ...
            try:
                ...
                z_list.append(
                    torch.from_numpy(data["v_star"]).to(f"cuda:{hparams.device}")
                )
                data_loaded = True
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        upd_matrix = torch.linalg.solve(
            ...
            @ (
                layer_ks.to(f"cuda:{hparams.device}")
                @ layer_ks.T.to(f"cuda:{hparams.device}")
                + cache_c[i, :, :].to(f"cuda:{hparams.device}")
            )
            + hparams.L2
            * torch.eye(
                layer_ks.shape[0], dtype=torch.float, device=f"cuda:{hparams.device}"
            ),
            P[i, :, :].to(f"cuda:{hparams.device}")
            @ layer_ks.to(f"cuda:{hparams.device}")
            @ resid.T.to(f"cuda:{hparams.device}"),
        )
