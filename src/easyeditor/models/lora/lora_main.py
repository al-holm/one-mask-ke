    ...
    for it in range(hparams.num_steps):
        ...
        for txt, tgt in zip(
            ...
            if "t5" in hparams.model_name.lower():
                ...
                target_ids = tok(tgt, return_tensors="pt", padding=True)[
                    "input_ids"
                ].to(device)
                ...
                unmasked_log_probs = (
                    ...
                    .gather(-1, inputs["labels"].unsqueeze(-1))
                    .squeeze(-1)
                )
                ...
                prompt_ids = tok(
                    list(txt), return_tensors="pt", padding=True, truncation=True
                )["input_ids"]
                num_prompt_toks = [
                ...
                for i in range(len(txt)):
                    tokens["labels"][i][
                        num_pad_toks[i] : num_pad_toks[i] + num_prompt_toks[i]
                    ] = mask_token
...
def apply_lora_to_multimodal_model(
    ...
    **kwargs: Any,
    ...
    edited_model = execute_multimodal_lora(
        model, tok, requests, hparams, keep_original_weight
    )
...
def execute_multimodal_lora(
    ...
    **kwargs: Any,
    ...
    for request in requests:
        print(
            f"Executing LoRA algo for: [{request['prompt']}] -> [{request['target']}]"
        )
    device = torch.device(f"cuda:{hparams.device}")
    # Define inputs
