...
def test_prediction_acc_LLM_judge(
    model, tok, hparams, prompt, target, device, locality=False
):
    ...
    if isinstance(model, T5ForConditionalGeneration):
        trunc_gen_tokens = gen_tokens[
            0
        ]  # encoder-decoder model only provied generated content after prompt
    else:
...
def test_prediction_acc(
    ...
    if (
        not locality
        and hasattr(hparams, "use_chat_template")
        and hparams.use_chat_template
    ):
...
def OOD_PPL(
    ...
    with torch.no_grad():
        logits = model(
            input_ids=input_ids.to(device), labels=target_ids.to(device)
        ).logits
    ...
    **kwargs,
    ...
    def generate_text(query, model, tokenizer):
        ...
        response = tokenizer.decode(
            outputs[0][len(src_input_ids) :], skip_special_tokens=True
        )
        return response
    ...
    if IKE:
        ...
        edit_text = clean_text(
            generate_text(kwargs["inner_q"], edited_model.model, tok)
        )
...
def test_safety_gen(
    model, tokenizer, test_prompt, cuda, max_tokens=1624, max_output_tokens=600
):
    ...
    if max_tokens < 1624:
        ...
        for item in test_prompt:
            ...
            if input["input_ids"].size(-1) > max_tokens - max_output_tokens:
                input = {
                    k: v[:, -(max_tokens - max_output_tokens) :]
        ...
        with torch.no_grad():
            ...
            only_response = [
                out[len(test_prompt[index]) + 1 :] for index, out in enumerate(texts)
            ]
        return only_response
