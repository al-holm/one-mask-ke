...
def compute_per_ike_metric(
    ...
    with torch.no_grad():
        ...
        result = {
            "es": es_per_icl(example, outer_base_logits, outer_edit_logits)[
                "acc_per"
            ].item(),
            "dd": kl_loc_loss(
                loc_base_logits, loc_edit_logits, example["loc_pre"]["q_mask"]
            ).item(),
        }
...
def compute_per_metric(
    ...
    with torch.no_grad():
        ...
        result = {
            ...
            "dd": kl_loc_loss(
                pre=loc_base_logits, post=loc_edit_logits, mask=kl_mask
            ).item(),
        }
