...
error_msg = {1: "Wrong data size", 2: "Wrong sample format", 3: "Wrong case id"}
...
def report_score(score, out_p):
    ...
    result["success"] = True
    total_score = (
        score["Edit_acc"]["final_score"] * 0.2
        + score["portability"]["final_score"] * 0.35
        + score["locality"]["final_score"] * 0.35
        + score["fluency"] * 0.1
    )
...
def sample_format(sample_list):
    ...
    for x in sample_list:
        ...
        if list(list1) != ["pre", "case_id", "requested_rewrite", "post"]:
            ...
            "locality_ans",
            "portability_ans",
        ] and list(list2) != ["rewrite_ans", "rephrase_ans", "portability_ans"]:
            tag = False
            break
        elif list(list3) != [
            "prompt",
...
def compute_acc(answers, outputs):
    ...
    temp_metrics["final_score"] = (
        temp_metrics["ROUGE-L"] + temp_metrics["Bert Score"]
    ) / 2
...
def eval_score(result_path):
    ...
    for item in data:
        if (
            "locality_ans" not in item["post"].keys()
            or len(item["requested_rewrite"]["locality"]["loc_hop"]["prompt"]) == 0
        ):
            continue
