...
class ADSDataset(BaseDataset):
        ...
        **kwargs,
        ...
        if "llava" in model_name.lower():
            ...
            raise AssertionError(
                "Not support the vision processor of {}".format(model_name)
            )
    ...
    def collate_fn(self, batch):
        ...
        for b in batch:
            temp_prompt, prompt_ids = self.multimodal_tokenize(
                b["type"], b["prompt"], b["target"], len(b["image"])
            )
            edit_inner["text_input"].append(temp_prompt)
            edit_inner["prompts_len"].append(len(prompt_ids[0]))

        edit_inner["labels"] = self.tok.tokenizer(
            trg,
        ...
        for b in batch:
            temp_prompt, prompt_ids = self.multimodal_tokenize(
                "text", b["locality_prompt"], b["locality_ground_truth"]
            )
            loc["text_input"].append(temp_prompt)
            loc["prompts_len"].append(len(prompt_ids[0]))

        loc["labels"] = self.tok.tokenizer(
            loc_a,
            add_special_tokens=False,
            return_tensors="pt",
        )["input_ids"]
        ...
        for b in batch:
            temp_prompt, prompt_ids = self.multimodal_tokenize(
                "image",
                b["multimodal_locality_prompt"],
                b["multimodal_locality_ground_truth"],
            )
            loc_image["text_input"].append(temp_prompt)
            loc_image["prompts_len"].append(len(prompt_ids[0]))
        loc_image["labels"] = self.tok.tokenizer(
            m_loc_a,
            add_special_tokens=False,
            return_tensors="pt",
        )["input_ids"]
