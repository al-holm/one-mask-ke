...
from ..utils import set_dropout, _logits, add_padding, add_sep
...
class SERAC(EditableModel):
    def __init__(
        ...
        if classifier is None:
            ...
            assert isinstance(classifier_tok, transformers.PreTrainedTokenizerBase), (
        ...
        if replacement is None:
            ...
            assert isinstance(replacement, torch.nn.Module), (
                "Rep is {type(replacement)}!"
            )
            assert isinstance(replacement_tok, transformers.PreTrainedTokenizerBase), (
    ...
    def build_rep_input_tokens(self, kwargs, idxs, generation=False):
        assert len(idxs) == len(kwargs["input_ids"]), (
...
class SERAC_MULTI(EditableModel):
    def __init__(
        ...
        if replacement is None:
            ...
            assert isinstance(replacement, torch.nn.Module), (
                f"Rep is {type(replacement)}!"
            )
            assert isinstance(replacement_tok, transformers.PreTrainedTokenizerBase), (
                f"Rep tok is {type(replacement_tok)}!"
            )
            self.replacement, self.replacement_tok = replacement, replacement_tok
        ...
        self.language_projection = torch.nn.Linear(
            self.model.Qformer.config.hidden_size, self.replacement.config.hidden_size
        )
        if cache_inputs is None:
            self.cache_inputs = []
            self.cache_labels = []
    ...
    def build_rep_input_tokens(self, kwargs, idxs, generation=False):
        ...
        rep_texts = [ctx + inp for ctx, inp in zip(selected_contexts, test_inputs)]
        rep_input_tokens = self.replacement_tok(
            rep_texts, return_tensors="pt", add_special_tokens=False
        ).to(self.config.device)
        ...
        if self.config.freeze_cntr:
            ...
            if self.config.model_name == "blip2":
                ...
                if image is not None:
                    ...
                    empty_targets = (
                        torch.ones(atts_img.shape, dtype=torch.long)
                        .to(image.device)
                        .fill_(-100)
                    )
                    targets = torch.cat([empty_targets, targets], dim=1)
